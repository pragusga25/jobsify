...
const start = async () => {
  ...
  try {
    await natsWrapper.connect('jobsify', 'adadfefefe', 'http://nats-srv:4222');
    ...
    new JobCreatedListener(natsWrapper.stan).listen();
    new JobDeletedListener(natsWrapper.stan).listen();
    new JobUpdatedListener(natsWrapper.stan).listen();
  ...
  app.listen(port, () => {
    console.log(`⚡️[applications]: Server is running on port ${port}`);
  });
};
